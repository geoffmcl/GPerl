...
sub get_vs_vars_bat() {
    my ($bfil);
    # 20201030 - Add VC16 in Dell-03
    $bfil = 'C:\Program Files (x86)\Microsoft Visual Studio\2019\community\VC\Auxiliary\Build\vcvarsall.bat';
    return $bfil if (-f $bfil);
    # 20180521 - Add VC14 in WIN7-PC
    $bfil = 'C:\Program Files (x86)\Microsoft Visual Studio 14.0\VC\vcvarsall.bat';
    return $bfil if (-f $bfil);
    # 15/04/2012 - Add VC10 in Win7-PC
    $bfil = 'C:\Program Files (x86)\Microsoft Visual Studio 10.0\VC\vcvarsall.bat';
    return $bfil if (-f $bfil);
...
sub secs_HHMMSS($) {
    ...
    if ($hour < 24) {
        ...
        return "$hour:$mins:$secs hh:mm:ss";
    }
    $days = int($hour / 24);
    $hour = $hour - ($days * 24);
    $hour = "0$hour" if ($hour < 10);
    return "$days days $hour:$mins:$secs hh:mm:ss";
}
